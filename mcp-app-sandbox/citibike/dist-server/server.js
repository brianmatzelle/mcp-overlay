...
import { findStation, searchStations } from "./lib/gbfs-fetcher.js";
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const DIST_DIR = path.join(__dirname, "dist");
const RESOURCE_URI = "ui://citibike-status/mcp-app.html";
function formatStationShortId(station) {
...
export function createServer() {
    ...
    registerAppTool(server, "citibike-status", {
        title: "Citi Bike Station Status",
        description: "Shows real-time Citi Bike station availability including classic bikes, " +
            "e-bikes, and open docks. Use station name (e.g. 'Grand Army Plaza', " +
            "'Bedford Ave'). Returns a live availability display.",
        inputSchema: {
            station: z
                .string()
                .describe("Station name or partial match (e.g. 'Grand Army Plaza', 'Bedford Ave & N 7 St')"),
        },
        _meta: {
        ...
        const classicBikes = match.num_bikes_available - match.num_ebikes_available;
        ...
        const summary = `${match.name}: ${Math.max(0, classicBikes)} classic, ${match.num_ebikes_available} e-bikes, ` +
    ...
    registerAppTool(server, "search-citibike", {
        title: "Search Citi Bike Stations",
        description: "Search for Citi Bike stations by name. Returns matching stations " +
        ...
        const text = results.length === 0
            ...
            : results
                .map((s) => `${s.name} (${formatStationShortId(s)}) - ${s.num_bikes_available} bikes, ${s.num_docks_available} docks`)
                .join("\n");
        return {
    ...
    // ── UI resource ────────────────────────────────────────────
    registerAppResource(server, "Citi Bike Station Status UI", RESOURCE_URI, {
