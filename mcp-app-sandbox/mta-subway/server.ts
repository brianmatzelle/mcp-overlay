...
export function createServer(): McpServer {
  ...
  registerAppTool(
    ...
    "subway-arrivals",
    {
      ...
      description:
        "Shows real-time subway arrival times for a NYC station. " +
      ...
      inputSchema: {
        line: z
          .string()
          .describe(
            "Subway line letter or number (e.g. 'G', 'L', '1', 'A')"
          ),
        station: z
          .string()
          .describe(
            "Station name or stop ID (e.g. 'Greenpoint Av', 'G26', 'Times Sq-42 St')"
          ),
      },
      _meta: {
    ...
    async ({ line, station }) => {
      ...
      if (!feedUrl) {
        return {
          content: [
            { type: "text" as const, text: `Unknown line: ${line}` },
          ],
          isError: true,
        };
      }
      ...
      const summary =
        `${upperLine} train at ${stationInfo.name}: ` +
  ...
  registerAppTool(
    ...
    "search-stations",
    {
      ...
      description:
        "Search for NYC subway stations by name. Returns matching stations " +
      ...
      const text =
        results.length === 0
          ...
          : results
              ...
              .map(
                (s) =>
                  `${s.name} (${s.id}) - Lines: ${s.lines.join(", ")}`
              )
              .join("\n");
  ...
  registerAppTool(
    ...
    "show-dashboard",
    {
      ...
      description:
        "Shows a dashboard with multiple subway arrival panels in a grid. " +
    ...
    async ({ panels }) => {
      ...
      const subwayHtml = await fs.readFile(
      ...
      const summary = panels
        .map(
          (p: { line: string; station: string; label?: string }) =>
            `${p.line} at ${p.station}${p.label ? ` (${p.label})` : ""}`,
        )
        .join(", ");
  ...
  registerAppResource(
    ...
    async () => {
      const html = await fs.readFile(
        path.join(DIST_DIR, "dashboard-app.html"),
        "utf-8",
      );
      return {
        contents: [
